...
---Reloads rooter plugin with the fixture path
...
function Harness:fixture(pattern)
  ...
  -- Reload rooter plugin with new path (runs in child process)
  self.child.lua(string.format([[
    -- Cleanup old rooter plugin if exists
    if _G.rooter_api and _G.rooter_api.cleanup then
      _G.rooter_api.cleanup()
    end
    ...
    -- Reload rooter plugin with fixture path
    _G.rooter_api = require("neodap").use(require("neodap.plugins.rooter"), {
...
function Harness:setup_neodap()
  ...
  self.child.lua(string.format([[
    ...
    neodap.use(require("neodap.plugins.task"))
    ...
    -- Load rooter plugin for :DapLaunch support
    -- Use parent's tmpdir so it finds the launch.json created by parent process
    _G.rooter_api = neodap.use(require("neodap.plugins.rooter"), {
      path = %q,
    })
...
---Root session is queryable via /rootSessions[N] (where N accounts for pre-existing roots)
---Child session is queryable via /rootSessions[N]/children[0]
---Uses jsdbg-simple fixture
---@param opts? { stopOnEntry?: boolean, focus_root?: boolean }
function Harness:start_jsdbg_dual(opts)
  ...
  -- Compute the expected indexes for the new sessions (handles pre-existing sessions)
  local base_idx = self:query_count("/sessions")
  self._jsdbg_root_idx = self:query_count("/rootSessions")

  -- Launch js-debug and focus root BEFORE child spawns (timing-critical for leaf_session).
  -- Must run in one child.lua block so focus happens synchronously before the event loop
  -- delivers the child session creation event.
  self.child.lua(string.format([[
    local fixtures = require("helpers.dap.fixtures")
    ...
    H.wait_for(10000, function()
      return _G.debugger:query("/sessions[%d]") ~= nil
    end)
    if %s then
      _G.debugger.ctx:focus(_G.debugger:query("/sessions[%d]").uri:get())
    end
  ]], program, opts.stopOnEntry and "true" or "false",
    base_idx,
    opts.focus_root and "true" or "false",
    base_idx))

  -- Wait for child to be stopped (debugger statement in fixture) via reactive URL query
  self:wait_url(self:_jsdbg_root_url() .. "/children(state=stopped)[0]", 10000)
end

---Get the URL for the jsdbg root session (accounts for pre-existing root sessions)
---@return string url
function Harness:_jsdbg_root_url()
  return string.format("/rootSessions[%d]", self._jsdbg_root_idx or 0)
end

---Get session URI by role (for JS-debug dual sessions)
---Queries the graph directly — no stored state
---@param role "root"|"child" Session role
---@return string? uri
function Harness:session_uri(role)
  local root_url = self:_jsdbg_root_url()
  if role == "root" then
    return self:query_uri(root_url)
  else
    return self:query_uri(root_url .. "/children[0]")
  end
end

---Clean up jsdbg dual session context
---Disconnects root which cascades to child
function Harness:cleanup_jsdbg_dual()
  local root_url = self:_jsdbg_root_url()
  if not self:query_is_nil(root_url) then
    local state = self:query_field(root_url, "state")
    if state ~= "terminated" then
      self:query_call(root_url, "disconnect")
    end
  end
...
function Harness:start_py_sleeper()
  ...
  -- Load rooter plugin with py-sleeper fixture (include resolve_adapter)
  self.child.lua(string.format([[
    local function resolve_adapter(config)
    ...
    require("neodap").use(require("neodap.plugins.rooter"), {
      path = %q,
      resolve_adapter = resolve_adapter,
    })
  ]], fixture_path))
...
function Harness:has_jsdbg_sessions()
  local root_url = self:_jsdbg_root_url()
  local has_root = not self:query_is_nil(root_url)
  local has_child = not self:query_is_nil(root_url .. "/children[0]")
  return has_root, has_child
end

---Focus the root session's first frame
function Harness:focus_root_session()
  local root_url = self:_jsdbg_root_url()
  self:wait_url(root_url .. "/threads[0]/stack/frames[0]")
  local frame_uri = self:query_uri(root_url .. "/threads[0]/stack/frames[0]")
  if frame_uri and frame_uri ~= vim.NIL then
    self:cmd("DapFocus " .. frame_uri)
  end
end

---Continue the child session
function Harness:continue_child_session()
  local child_url = self:_jsdbg_root_url() .. "/children[0]"
  local state = self:query_field(child_url, "state")
  if state == "stopped" then
    self:query_call(child_url, "fetchThreads")
    self:wait_url(child_url .. "/threads[0]")
    self:query_call(child_url .. "/threads[0]", "continue")
  end
end
...
function Harness:wait_child_terminated(timeout)
  self:wait_field(self:_jsdbg_root_url() .. "/children[0]", "state", "terminated", timeout or M.TIMEOUT.MEDIUM)
end

---Terminate the first session
---For js-debug dual sessions, use cleanup_jsdbg_dual() instead
function Harness:terminate_root_session()
  if not self:query_is_nil("/sessions[0]") then
    self:query_call("/sessions[0]", "terminate")
    self:wait_field("/sessions[0]", "state", "terminated", M.TIMEOUT.SHORT)
  end
end
...
function Harness:child_format_includes_root_name()
  local root_url = self:_jsdbg_root_url()
  local child_url = root_url .. "/children[0]"
  self.child.lua(string.format([[
    _G._format_ok = false
    local child = _G.debugger:query(%q)
    local root = _G.debugger:query(%q)
...
function Harness:wait_context_frame(timeout)
  return self:wait_url("@frame", timeout or 5000)
end
...
---Wait for multiple URLs to all resolve in a single atomic vim.wait.
---Avoids race conditions where filtered view indexes shift between separate wait_url calls.
---@param urls string[] URLs that must all resolve
---@param timeout? number Timeout in ms (default 5000)
---@return boolean success True if all URLs resolved before timeout
function Harness:wait_urls(urls, timeout)
  timeout = timeout or 5000
  local checks = {}
  for _, url in ipairs(urls) do
    table.insert(checks, string.format(
      '_G.debugger:query(%q) ~= nil', url
    ))
  end
  self.child.lua(string.format([[
    _G._wait_urls_ok = vim.wait(%d, function()
      return %s
    end, 10)
  ]], timeout, table.concat(checks, " and ")))
  return self:get("_G._wait_urls_ok") or false
end

---Wait for multiple URLs to all resolve, then focus the entity at focus_url.
---Combines wait + focus in a single RPC to avoid TOCTOU race on filtered indexes.
---@param urls string[] URLs that must all resolve before proceeding
---@param focus_url string URL of the entity to focus after all URLs resolve
---@param timeout? number Timeout in ms (default 5000)
---@return boolean success True if all URLs resolved and focus succeeded
function Harness:wait_and_focus(urls, focus_url, timeout)
  timeout = timeout or 5000
  local checks = {}
  for _, url in ipairs(urls) do
    table.insert(checks, string.format(
  ...
  self.child.lua(string.format([[
    _G._wait_focus_ok = vim.wait(%d, function()
      return %s
    end, 10)
    if _G._wait_focus_ok then
      local entity = _G.debugger:query(%q)
      if entity then _G.debugger.ctx:focus(entity.uri:get()) end
    end
  ]], timeout, table.concat(checks, " and "), focus_url))
  return self:get("_G._wait_focus_ok") or false
end
...
-------------------------------------------------------------------------------
-- Mock Task Runner Helpers
-------------------------------------------------------------------------------

---Setup a mock task runner in the child Neovim.
---Records task names to _G._task_log (queryable via h:get()).
---Tasks succeed by default unless added to _G._failing_tasks.
---@param opts? { failing_tasks?: table<string, boolean> }
function Harness:setup_mock_task_runner(opts)
  opts = opts or {}
  local failing_tasks = opts.failing_tasks or {}
  local failing_str = vim.inspect(failing_tasks)
  self.child.lua(string.format([[
    _G._task_log = {}
    _G._failing_tasks = %s
    ...
    task_runner.register({
      name = "mock",
      run = function(name, run_opts)
        table.insert(_G._task_log, name)
        if _G._failing_tasks[name] then
          return false
        end
        return true
      end,
...
---Get the task log from the mock task runner.
---@return string[] List of task names that were executed
function Harness:get_task_log()
  local result = self:get("_G._task_log")
  if result == nil or result == vim.NIL then return {} end
  return result
end

---Clear the task log in the mock task runner.
function Harness:clear_task_log()
  self.child.lua([[ _G._task_log = {} ]])
end

---Set which tasks should fail in the mock runner.
---@param tasks table<string, boolean> Map of task name -> should fail
function Harness:set_failing_tasks(tasks)
  local str = vim.inspect(tasks)
  self.child.lua(string.format([[ _G._failing_tasks = %s ]], str))
end

---Setup vim.ui.select mock in the child Neovim.
---Auto-responds with the configured choice. Records calls to _G._ui_select_log.
---@param choice? string The choice to auto-select (nil = cancel)
function Harness:setup_ui_select_mock(choice)
  local choice_str = choice and string.format("%q", choice) or "nil"
  self.child.lua(string.format([[
    _G._ui_select_log = {}
    _G._ui_select_choice = %s

    vim.ui.select = function(items, opts, on_choice)
      table.insert(_G._ui_select_log, {
        items = items,
        prompt = opts and opts.prompt or nil,
      })
      -- Find matching item
      local selected = nil
      if _G._ui_select_choice then
        for _, item in ipairs(items) do
          if item == _G._ui_select_choice then
            selected = item
            break
          end
        end
...
---Get the ui.select log from the mock.
---@return table[] List of { items: string[], prompt: string? }
function Harness:get_ui_select_log()
  local result = self:get("_G._ui_select_log")
  if result == nil or result == vim.NIL then return {} end
  return result
end

---Wait for the task log to reach a target count.
---@param count number Expected number of task entries
---@param timeout? number Timeout in ms (default 5000)
---@return boolean success
function Harness:wait_task_log(count, timeout)
  timeout = timeout or 5000
  self.child.lua(string.format([[
    _G._wait_task_ok = vim.wait(%d, function()
      return #(_G._task_log or {}) >= %d
    end, 10)
  ]], timeout, count))
  return self:get("_G._wait_task_ok") or false
end
