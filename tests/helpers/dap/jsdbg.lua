...
function M.start_session(opts)
  ...
  -- Start root session (fire-and-forget: returns nil for server adapters
  -- because session creation happens asynchronously via supervisor callback)
  debugger:debug({
  ...
  -- Wait for root session to appear in the graph
  local root_session
  H.wait_for(10000, function()
    root_session = H.edge_first(debugger.sessions)
    return root_session ~= nil
  end)
