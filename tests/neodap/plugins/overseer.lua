...
local T = harness.integration("overseer", function(T, ctx)
  ...
  T["postDebugTask runs after session terminates"] = function()
    -- Skip for JavaScript: debug() returns nil for server adapters,
    -- so the overseer postDebugTask listener is never attached
    ...
    -- Poll for postDebugTask completion (reactive wait instead of blind sleep)
    h.child.lua([[
      vim.wait(5000, function()
        return #(vim.g.overseer_task_ran or {}) >= 1
      end, 50)
    ]])
  ...
  T["both preLaunchTask and postDebugTask work together"] = function()
    ...
    h.child.lua([[
      vim.wait(5000, function()
        return #(vim.g.overseer_task_ran or {}) >= 2
      end, 50)
    ]])
