...
local T = harness.integration("tree_buffer_output", function(T, ctx)
  ...
  T["multiple outputs stream into tree as stepping"] = function()
    ...
    if ctx.adapter_name == "python" then
      -- Python: debugpy's non-deterministic output batching means consecutive
      -- "Step N" outputs may be collapsed (×N badge) since they all normalize
      -- to the same signature "Step <N>". Count visible steps + collapsed entries.
      local content = h:buffer_content()
      local visible_steps = 0
      for _ in content:gmatch("Step %d") do
        visible_steps = visible_steps + 1
      end
      local collapsed_extra = 0
      for n in content:gmatch("×(%d+)") do
        collapsed_extra = collapsed_extra + tonumber(n) - 1
      end
      local total = visible_steps + collapsed_extra
      if total < 3 then
        error(string.format(
          "Output tree should show all 3 steps (%d found: %d visible + %d collapsed)\n" ..
          "Buffer content:\n%s",
          total, visible_steps, collapsed_extra, content
        ))
      end
    else
      -- Wait for UI to settle (js-debug output timing)
