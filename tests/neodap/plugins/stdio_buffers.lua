...
local T = harness.integration("stdio_buffers", function(T, ctx)
  T["DapOpen replace output opens log file buffer"] = function()
    local h = ctx.create()
    h:fixture("simple-vars")
    h:use_plugin("neodap.plugins.stdio_buffers")
    h:use_plugin("neodap.plugins.open_cmd")

    h:cmd("DapLaunch Debug stop")
    h:wait_url("/sessions/threads/stacks[0]/frames[0]")
    h:cmd("DapFocus /sessions/threads/stacks[0]/frames[0]")

    -- Open output in current window
    h.child.cmd("DapOpen replace output")
    h:wait(100)
  ...
  T["output log receives output after stepping"] = function()
    ...
    -- Open output buffer in current window
    h.child.cmd("DapOpen replace output")
    h:wait(100)
