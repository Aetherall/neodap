...
local T = harness.integration("tree_buffer_reactivity", function(T, ctx)
  ...
  T["session state updates reactively when terminated"] = function()
    ...
    -- Verify the tree still renders after termination (session is still visible)
    -- The stop icon (⏹) is now right-aligned virtual text, so check the session name is present
    -- and the tree updated reactively (Output/Threads groups still visible)
    h:assert_buffer_contains("Output", "Session tree should still show Output group after termination")
  end
