...
T["dap_exception_api"]["toggle changes enabled state"] = function()
  ...
  local filter_id = h:query_field("@session/exceptionFilters[0]", "filterId")
  local filter_url = string.format("@session/exceptionFilters(filterId=%s)[0]", filter_id)
...
T["dap_exception_api"]["set_condition updates condition"] = function()
  ...
  MiniTest.expect.equality(h:query_field("@session/exceptionFilters(filterId=" .. filter_id .. ")[0]", "condition"), "x > 0")
end
