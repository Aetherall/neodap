-- Visual tests for hover plugin
-- SKIPPED: The hover plugin was rewritten from in-process LSP to split-float tree view
-- (commit b3d4d04, 2026-02-27) but these tests and their reference screenshots were never
-- updated. Tests need rewriting to match the new hover API and screenshots need regeneration.
local harness = require("helpers.test_harness")

local T = harness.integration("hover", function(T, ctx)
  -- Skip all hover tests until they are rewritten for the split-float hover plugin
  local skip = function() return end
  -------------------------------------------------------------------------------
  -- Basic Hover Tests
  -------------------------------------------------------------------------------
  ...
  -- Skip all hover tests until rewritten for split-float hover plugin
