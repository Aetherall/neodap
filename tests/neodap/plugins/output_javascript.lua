...
T["dap_output"]["output event creates entity"] = function()
  ...
  MiniTest.expect.equality(h:query_count("/rootSessions[0]/children[0]/outputs") > 0, true)
end
...
T["dap_output"]["output entities have sequential seq values"] = function()
  ...
  local count = h:query_count("/rootSessions[0]/children[0]/outputs")
  if count >= 2 then
    local seq_values = {}
    for i = 0, count - 1 do
      local seq = h:query_field(string.format("/rootSessions[0]/children[0]/outputs[%d]", i), "seq")
