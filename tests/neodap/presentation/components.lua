...
return harness.integration("components", function(T, ctx)
  ...
  T["variable value shows value"] = function()
    ...
    -- Value may be a multi-segment result (TreeSitter/tokenize_output highlighting)
    -- or a single { text, hl } depending on what's available
    local has_content = value.text ~= nil or (value.segments and #value.segments > 0)
