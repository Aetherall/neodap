...
local T = harness.integration("prelaunch_tasks", function(T, ctx)
  ...
  T["failed compound preLaunchTask with Cancel aborts launch"] = function()
    local h = ctx.create()
    local fixture_path = h:fixture("multi-session")
    h:use_plugin("neodap.plugins.overseer")
    setup_task_tracking(h, fixture_path)
    ...
    h:cmd("DapLaunch Both with failing compound task")

    -- Wait for task to fail and dialog to resolve
    h:wait(2000)

    -- Task should have run and failed
    local tasks = get_ran_tasks(h)
    MiniTest.expect.equality(#tasks >= 1, true, "Expected build-fail task to run")
    local fail_task = find_task(tasks, "build-fail")
    MiniTest.expect.equality(fail_task ~= nil, true, "Expected build-fail task")
    MiniTest.expect.equality(fail_task.status, "FAILURE")

    -- No sessions should have been created (Cancel aborts)
    MiniTest.expect.equality(h:query_count("/sessions"), 0)
  end

  T["failed compound preLaunchTask with Launch anyway proceeds"] = function()
    local h = ctx.create()
    local fixture_path = h:fixture("multi-session")
    h:use_plugin("neodap.plugins.overseer")
    setup_task_tracking(h, fixture_path)
    ...
    -- Sessions should be created despite task failure
    ...
    -- But sessions were created anyway
  ...
  T["DapRestartConfig re-runs per-config preLaunchTask"] = function()
    ...
    -- Initial tasks ran
    local initial_tasks = get_ran_tasks(h)
    MiniTest.expect.equality(#initial_tasks >= 2, true, "Expected 2 initial preLaunchTasks")
    MiniTest.expect.equality(find_task(initial_tasks, "build-a") ~= nil, true)
    MiniTest.expect.equality(find_task(initial_tasks, "build-b") ~= nil, true)
    ...
    -- Tasks should have run again (total: 4 = 2 initial + 2 restart)
    h.child.lua([[
      vim.wait(5000, function()
        return #(vim.g.overseer_tasks_ran or {}) >= 4
      end, 50)
    ]])

    local final_tasks = get_ran_tasks(h)
    MiniTest.expect.equality(#final_tasks >= 4, true,
      "Expected 4 total tasks (2 initial + 2 restart), got " .. #final_tasks)

    -- Count occurrences
    local counts = {}
    for _, task in ipairs(final_tasks) do
      counts[task.name] = (counts[task.name] or 0) + 1
    end
    MiniTest.expect.equality(counts["build-a"], 2, "Expected build-a to run twice")
    MiniTest.expect.equality(counts["build-b"], 2, "Expected build-b to run twice")
  end
