...
T["leaf_session"]["parent regains focus when child terminates"] = function()
  ...
  -- In js-debug, child termination cascades to root termination.
  -- leaf_session moves focus to root transiently, but root terminates quickly.
  -- Verify both sessions terminated properly (no need to call terminate_root_session).
  h:wait_field(h:session_uri("root"), "state", "terminated")
end
