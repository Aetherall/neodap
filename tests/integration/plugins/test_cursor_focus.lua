...
return harness.integration("cursor_focus", function(T, ctx)
  ...
  T["DapJump @frame works after step with cursor_focus"] = function()
    ...
    -- Extended timeout: step + stopped event + stack trace fetch can be slow under load
    h:cmd("DapStep over")
    h:wait_url("/sessions/threads(state=stopped)/stacks[0]/frames(line=2)[0]", 15000)
    h:cmd("DapFocus /sessions/threads/stacks[0]/frames[0]")
  ...
  T["cursor_focus handles multiple sessions"] = function()
    ...
    -- Extended timeout: two sequential debugpy sessions under parallel load need more time
    h:cmd("DapLaunch Debug stop")
    h:wait_url("/sessions[1]/threads(state=stopped)/stacks[0]/frames[0]", 15000)
