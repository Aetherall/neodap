...
return function(Config)
  ...
  ---Check if this Config has been superseded by a newer Config with the same name.
  ---Called when a Config transitions to "terminated". If a newer Config exists
  ---(higher index, same name), marks this one as superseded so it hides from
  ---the default tree view.
  function Config:checkSuperseded()
    local my_index = self.index:get() or 0
    local my_name = self.name:get()
    ...
    for cfg in debugger.configs:filter({
      filters = {{ field = "name", op = "eq", value = my_name }}
    }):iter() do
      if (cfg.index:get() or 0) > my_index then
        self:update({ superseded = true })
        return
      end
    end
  end
