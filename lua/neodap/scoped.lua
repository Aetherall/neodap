...
function Scope:cancel()
  ...
  -- Use pairs (not ipairs) because children is a weak-value table —
  -- GC can create gaps in the integer keys that ipairs would miss
...
-- Each effect invocation gets its own child scope that is cancelled
-- when the effect re-runs or the subscription is cleaned up.
-- This makes scoped.schedule() work inside use() effects — scheduled
-- callbacks auto-cancel when the effect re-runs or the scope ends.
local function patch_signal(signal)
  ...
  function signal:use(effect)
    ...
    local invocation_scope = nil

    local function wrapped_effect(value)
      -- Cancel previous invocation scope (handles user cleanup,
      -- nested subscriptions, and pending scoped.schedule callbacks)
      if invocation_scope then
        invocation_scope:cancel()
      end

      -- Create fresh invocation scope as child of subscription scope
      invocation_scope = Scope.new(scope)

      -- Run effect in invocation scope so M.current() / scoped.schedule()
      -- capture it, and nested use()/each() register with it
      table.insert(scope_stack, invocation_scope)
      local ok, result = pcall(effect, value)
      table.remove(scope_stack)

      if ok and type(result) == "function" then
        invocation_scope:onCleanup(result)
      elseif not ok then
        error(result, 0)
      end
    end
...
function M.flatMap(source, transform)
  ...
  function flat:use(effect)
    ...
    local function runEffect(value)
      ...
      -- Run effect in invocation scope so M.current() / a.run()
      -- capture it, and nested use()/each() register with it
      table.insert(scope_stack, invocation_scope)
      local ok, result = pcall(effect, value)
      table.remove(scope_stack)
      ...
      elseif not ok then
        vim.schedule(function()
          vim.notify("[neodap] flatMap effect error: " .. tostring(result), vim.log.levels.ERROR)
        end)
      end
    end
...
---Schedule a callback that auto-cancels if the scope is done.
---Use this instead of vim.schedule() inside reactive contexts (each(), use())
---to prevent stale side effects when cleanup fires before the callback runs.
---@param fn function The callback to schedule
function M.schedule(fn)
  local scope = M.current()
  vim.schedule(function()
    if not scope:done() then
      fn()
    end
  end)
end

---Instance method: schedule a callback guarded by this scope's lifecycle.
---@param fn function The callback to schedule
function Scope:schedule(fn)
  vim.schedule(function()
