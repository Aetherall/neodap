...
local CAT_HL = { stderr = "DapStopped", stdout = "DapRunning", console = "DapState", repl = "DapRepl" }

-- Output category -> text highlight
local CAT_TEXT_HL = { stderr = "DapOutputStderr", stdout = "DapOutputStdout", console = "DapOutputConsole", repl = "DapOutputRepl" }
...
function M.register(debugger)
  ...
  rc(debugger, "value", "Variable", function(var)
    ...
    -- Try TreeSitter first (full syntax highlighting for complex values)
    -- Falls back to tokenize_output (basic token highlighting) when TS queries unavailable
    local ts_hl = require("neodap.presentation.ts_highlight")
    local segs = ts_hl.highlight(val, "javascript", "DapVarValue")
    if #segs > 1 then return { segments = segs } end
    -- Single segment means TS didn't produce highlights — try tokenize_output
    local tok_segs = tokenize_output(val, "DapVarValue")
    if #tok_segs > 1 then return { segments = tok_segs } end
    return segs[1] or tok_segs[1] or { text = val, hl = "DapVarValue" }
  end)
  ...
  rc(debugger, "repeat_badge", "Output", function(output)
    local count = output.repeatCount and output.repeatCount:get()
    if not count or count <= 1 then return nil end
    return { text = " ×" .. count, hl = "DapRepeatBadge" }
  end)
