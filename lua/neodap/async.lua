...
-- Default error handler - routes unhandled async errors through E.report().
-- E.report() is the SINGLE place where vim.notify is called for errors.
-- Raw strings and NeodapErrors with show_user ~= false are shown to the user.
...
-- Uses standalone context (parent_ctx=false) to avoid inheriting cancelled
-- reactive scopes — a.fn is a synchronous wrapper, not a long-running task.
...
function M.fn(fn)
  return function(...)
    ...
    local args = { ... }
    local result
    M.run(function() result = fn(unpack(args)) end, nil, false)
    return result
  end
end
...
function M.memoize(fn)
  ...
  return function(self)
    ...
    if M.context() then
      ...
      -- Clean up cache if our context is cancelled mid-yield
      local ctx = contexts[coroutine.running()]
      if ctx then
        ctx:onCleanup(function()
          if cache[self] == entry then
            cache[self] = nil
            for _, cb in ipairs(entry.waiters) do cb("cancelled") end
          end
        end)
      end
    ...
    -- Clean up cache if the task's context is cancelled
    task:onCleanup(function()
      if cache[self] == entry then
        cache[self] = nil
        for _, cb in ipairs(entry.waiters) do cb("cancelled") end
      end
