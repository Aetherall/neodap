    ...
    vim.schedule(function()
      ...
      if not success then
        ...
        if panic_mode then
          ...
          -- error("NvimAsync coroutine error: " .. error_msg)
          vim.cmd("qa!") -- Quit all
        else
...
function NvimAsync.has_pending_tasks()
  -- Check if there are any pending tasks in the NvimAsync registry
  for co, _ in pairs(nvim_async_coroutines) do
    local status = coroutine.status(co)
    print("NVIM_ASYNC: Checking coroutine status:", status)
    if status ~= "dead" then
      return true
    end
  end
  print("NVIM_ASYNC: No pending tasks found")
  return false
end
