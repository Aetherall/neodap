...
---Each invocation gets its own child scope so that nested a.run() coroutines
---and scoped.schedule() callbacks are automatically cancelled when the effect
---re-runs or the subscription is cleaned up.
...
function Accessor:use(callback)
  ...
  local function update()
    ...
    -- nested subscriptions, pending a.run() coroutines, and scoped.schedule callbacks)
    if invocation_scope then
      invocation_scope:cancel()
    end

    -- Create fresh invocation scope as child of subscription scope
    invocation_scope = scoped.Scope.new(scope)

    -- Resolve entity
    local entity = self:get()

    -- Run callback in invocation scope so scoped.current() / a.run()
    -- capture it, and nested use()/each() register with it
    local ok, result = pcall(scoped.withScope, invocation_scope, function()
