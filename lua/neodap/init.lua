...
local function create_debugger_instance(graph, parent_scope)
  ...
  -- Lifecycle hook registry (per-instance, cleaned up on dispose)
  debugger._hooks = {}

  ---Register a lifecycle hook
  ---@param event string Hook event name ("before_debug", "on_session", etc.)
  ---@param handler function Hook handler function
  function debugger:hook(event, handler)
    if not self._hooks[event] then
      self._hooks[event] = {}
    end
    table.insert(self._hooks[event], handler)
  end
