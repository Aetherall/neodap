...
local function make_defaults(get_state, toggle_expand, render_buffer, show_root, get_context)
  ...
  local defaults = {
    ...
    ["?"] = function(ctx)
      help.toggle(ctx.bufnr, function()
        local c = get_context(ctx.bufnr)
        return c.item and c.item.type or nil
      end)
    end,
    ...
    -- Toggle show/hide superseded (terminated) configs
    ["T"] = function(ctx)
      local show = not (rawget(ctx.debugger, "_show_terminated") or false)
      rawset(ctx.debugger, "_show_terminated", show)
...
local function setup(bufnr, config, defaults, get_context)
  ...
  local function make_handler(key, default)
    return function()
      ...
      if not ok then E.report(err) end
    end
  end
  for key, h in pairs(defaults) do
    local desc = help.get_desc(key)
    vim.keymap.set("n", key, make_handler(key, h), vim.tbl_extend("force", opts, { desc = "DAP Tree: " .. (desc or key) }))
  end
  for key, _ in pairs(config.keybinds) do
    if not defaults[key] then
      local desc = help.get_desc(key)
      vim.keymap.set("n", key, make_handler(key, nil), vim.tbl_extend("force", opts, { desc = "DAP Tree: " .. (desc or key) }))
    end
  end
end
