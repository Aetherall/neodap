...
local function fetch_scopes(entity, context)
  if entity:isSessionTerminated() then return end
  -- When entering the view (not user-initiated expand), only fetch scopes for
  -- the focused frame. This avoids flooding the adapter with requests for
  -- jest/async frames that can't provide variables ("Variables not available in
  -- async stacks"), which would otherwise produce error notifications on every step.
  if context == "enter" and not (entity.focused and entity.focused:get()) then return end
  entity:fetchScopes()
end
...
local STACK = { frames = { edges = FRAME } }
...
-- Console (chronological): same filters but sorted oldest-first (ascending globalSeq)
local CONSOLE_ASC = {
  allOutputs = {
    eager = true,
...
-- Build Configs group edges dynamically based on current viewMode and show_terminated settings
-- Called when building the Debugger tree query
---@param debugger? table Debugger entity for querying active configs
---@param show_terminated? boolean Whether to show superseded/terminated configs
---@return table CONFIGS_GROUP edges
local function build_configs_group_edges(debugger, show_terminated)
  ...
  -- Use visibleConfigs (hides superseded) by default, configs (all) when toggled
  local configs_edge = show_terminated and "configs" or "visibleConfigs"
  return { debuggers = { inline = true, eager = true, edges = {
    [configs_edge] = { eager = true, edges = config_edges },
  }}}
end

-- Default Configs group (targets view, hides superseded)
local CONFIGS_GROUP = { debuggers = { inline = true, eager = true, edges = {
  visibleConfigs = { eager = true, edges = CONFIG_TARGETS },
}}}
...
---@param opts? { show_terminated?: boolean } Optional display options
---@return table query View query configuration
local function build_query(root_type, root_uri, edge_type, entity, opts)
  opts = opts or {}
  local query_edges
  ...
  elseif root_type == "Debugger" and entity and not edge_type then
    -- For Debugger, build Configs group edges dynamically based on viewMode + show_terminated
    local configs_group = build_configs_group_edges(entity, opts.show_terminated)
    query_edges = {
      configsGroups = { eager = true, edges = configs_group },
      breakpointsGroups = { eager = true, edges = BREAKPOINTS_GROUP },
      exceptionFiltersGroups = { eager = true, edges = EXCEPTION_FILTERS_GROUP },
