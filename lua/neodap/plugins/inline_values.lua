...
return function(debugger, config)
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    callback = function(ev)
      ...
      local buffer_scope = scoped.bufferScope(bufnr, plugin_scope, "BufWipeout")
      ...
      scoped.withScope(buffer_scope, function()
        debugger.ctx.frame:use(function()
          ...
          for id, lines in pairs(id_lines) do
            -- Evaluate errors are expected here (variable not in scope at current frame).
            -- Use an explicit callback to log at debug level instead of routing through
            -- default_error_handler → E.report → vim.notify, which would spam the user
            -- with ~8 error popups on every step.
            local task = a.run(function()
              ...
              -- Yield to main thread for vim API access (cancellation handled by a.wait)
              a.wait(a.main)
              if not vim.api.nvim_buf_is_valid(bufnr) then return end
              ...
              if err then log:debug("inline_values: evaluate failed", { id = id, error = tostring(err) }) end
            end)
  ...
  E.create_command("DapInlineValues", function()
