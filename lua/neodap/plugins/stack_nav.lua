...
return function(debugger, config)
  ...
  function api.up()
    ...
    error(E.warn("DapUp: No caller frame (already at outermost frame)"), 0)
    return false
  end
  ...
  function api.down()
    ...
    error(E.warn("DapDown: No callee frame (already at innermost frame)"), 0)
    return false
  end
  ...
  function api.top()
    ...
    if #frames == 0 then
    error(E.warn("DapTop: No frames in stack"), 0)
    return false
    end
  ...
  E.create_command("DapUp", function() api.up() end, {
    desc = "Focus caller frame (up the stack)",
  })

  E.create_command("DapDown", function() api.down() end, {
    desc = "Focus callee frame (down the stack)",
  })

  E.create_command("DapTop", function() api.top() end, {
