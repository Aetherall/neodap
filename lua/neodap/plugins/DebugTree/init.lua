...
local function formatVariableValue(ref, style)
  ...
  -- Normalize and truncate string values
  if type(value) == "string" then
    -- Replace carriage returns and newlines with spaces: [\r\n]+
    -- Replace escaped newlines, returns, tabs with spaces: \\[nrt] 
    -- Collapse multiple whitespace into single space: %s+
    -- Trim leading/trailing whitespace: ^%s*(.-)%s*$
    value = value:gsub("[\r\n]+", " "):gsub("\\[nrt]", " "):gsub("%s+", " "):match("^%s*(.-)%s*$") or ""
  ...
  if var_type == "function" then
    ...
    -- Match function declarations: ^function (starts with "function")
    if value:match("^function") then
      -- Extract function signature between "function" and "{": ^function%s*([^{]*)
      -- %s* matches optional whitespace after "function"
      -- ([^{]*) captures everything up to the first "{"
      local signature = value:match("^function%s*([^{]*)")
      return signature and ("ƒ " .. signature:gsub("%s+", " "):sub(1, 20) .. (signature:len() > 20 and "..." or "")) or "ƒ (...)"
    end
    
    -- Match already formatted functions: ^ƒ (starts with function symbol)
    if value:match("^ƒ") then
      return value
    end
  ...
  if var_type == "object" and ref.variablesReference and ref.variablesReference > 0 then
    -- Handle arrays
    if isArray(ref) then
      -- Extract array length from formats like "(5)" or "Array(5)":
      -- ^%((%d+)%) matches "(number)" at start of string
      -- ^Array%((%d+)%) matches "Array(number)" at start of string
      -- (%d+) captures one or more digits
      local length = value:match("^%((%d+)%)") or value:match("^Array%((%d+)%)")
      return length and ("(" .. length .. ") [...]") or "[...]"
    end
    
    -- Handle objects - check if value looks like JSON object: ^%{.*%}$
    -- ^%{ matches "{" at start, %} matches "}" at end, .* matches anything between
...
---@param self api.Session
---@return table[] Content parts for rendering
function Session:asNodeLine()
  ...
  return {
    { text = "📡 " .. display_text, highlight = "@namespace" }
  }
end
...
---@param self api.Thread
---@return table[] Content parts for rendering
function Thread:asNodeLine()
  local status_icon = self.stopped and "⏸️" or "▶️"
  local status_text = self.stopped and "stopped" or "running"
  local thread_text = " Thread " .. tostring(self.id) .. " (" .. status_text .. ")"
  
  return {
    { text = status_icon, highlight = "@text.note" },
    { text = thread_text, highlight = "@function.call" }
  }
end
...
---@param self api.Stack
---@return table[] Content parts for rendering
function Stack:asNodeLine()
  local frames = self:getFrames()
  local frame_count = frames and frames:count() or 0
  
  return {
    { text = "📚 Stack (" .. frame_count .. " frames)", highlight = "@type" }
  }
end
...
---@return table[] Content parts for rendering
function Frame:asNodeLine(index)
  ...
  if index then
    table.insert(parts, { text = "#" .. index .. " ", highlight = "@number" })
  end
  
  table.insert(parts, { text = "🖼️  ", highlight = "@text.note" })
  table.insert(parts, { text = frame_name, highlight = "@function" })
...
function Scope:asNodeLine()
  ...
  return {
    { text = scope_icon .. " " .. scope_name, highlight = "@parameter" }
  }
end
...
function Variable:asNodeLine()
  ...
  -- Trust DAP semantic data first (authoritative)
  local name_highlight = "@variable"  -- Default

  -- 1. Use DAP kind information (debugger knows the actual semantics)
  if kind then
    if kind == "method" then
  ...
  -- 2. Use DAP attributes (debugger knows actual properties)
  if attributes then
  ...
  -- 3. Use DAP visibility (debugger knows actual visibility)
  if visibility then
    if visibility == "private" and not kind then
      name_highlight = "@field"
    elseif visibility == "internal" then
      name_highlight = "@variable.builtin"
    end
  end

  -- 4. Use scope context (debugger knows actual scope)
  if self.scope then
    local scope_name = self.scope.name
    if scope_name == "Arguments" and not kind then
      name_highlight = "@parameter"
    elseif scope_name == "Global" and not kind then
      name_highlight = "@variable.builtin"
    end
  end

  local parts = {
    { text = icon .. " ", highlight = "@text.note" },
    { text = var_name, highlight = name_highlight },
    { text = ": ", highlight = "@punctuation.delimiter" },
    { text = formatted_value, highlight = style.highlight }
  }

  -- Add memory reference if available
  if self.ref and self.ref.memoryReference then
    table.insert(parts, { text = " @", highlight = "@comment" })
    table.insert(parts, { text = self.ref.memoryReference, highlight = "@number" })
  end
...
function DebugTree:setupViewTreeKeybindings(view_tree, popup, title)
  ...
  popup:map("n", "o", function() self:Expand(view_tree) end)                         -- o: expand only
...
function DebugTree:setupTreeRendering(tree)
  ...
  tree._.prepare_node = function(node)
    ...
    -- Get content from the DAP entity using asNodeLine()
    if node._dap and node._dap.asNodeLine then
      local content_parts
      if node.type == "frame" and node._frame_index then
        content_parts = node._dap:asNodeLine(node._frame_index)
      else
        content_parts = node._dap:asNodeLine()
      end
      
      -- Add each content part with its highlight
      for _, part in ipairs(content_parts) do
        line:append(part.text, part.highlight)
      end
    else
      -- Fallback for nodes without asNodeLine method
