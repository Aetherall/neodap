...
return function(debugger)
  ...
  function api.run_to_cursor(opts)
    ...
    if loc.path == "" then
      error(E.warn("DapRunToCursor: Buffer has no file path"), 0)
    end

    local thread = debugger.ctx.thread:get()
    if not thread then
      error(E.warn("DapRunToCursor: No focused thread"), 0)
    end

    local source = debugger:getOrCreateSource(loc)
    local temp_bp = source:addBreakpoint({ line = loc.line })
    if not temp_bp then
      error(E.warn("DapRunToCursor: Failed to create breakpoint"), 0)
    end
  ...
  E.create_command("DapRunToCursor", function(cmd_opts)
