...
return function(debugger, config)
  ...
  local function create_terminal()
    ...
    if not start_server() then
      E.report(E.user("Failed to start js-debug terminal server"))
      return nil
    end
  ...
  local function enable_in_terminal(bufnr)
    ...
    if vim.bo[bufnr].buftype ~= "terminal" then
      E.report(E.user("Buffer is not a terminal"))
      return false
    end
    ...
    if not jobid then
      E.report(E.user("Could not get terminal job ID"))
      return false
    end
  ...
  E.create_command("DapJsDebugTerminal", function()
    create_terminal()
  end, {
    desc = "Create a js-debug terminal for automatic Node.js debugging",
  })

  E.create_command("DapJsDebugTerminalEnable", function()
    enable_in_terminal()
  end, {
