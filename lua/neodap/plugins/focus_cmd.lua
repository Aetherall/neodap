...
return function(debugger)
  ...
  function api.focus(url, callback)
    if not url or url == "" then
      local err = E.warn("DapFocus: Missing URL")
    ...
    picker:resolve(url, function(entity)
      if not entity then
        E.report(E.warn("DapFocus: No entity found for '" .. url .. "'"))
        if callback then callback(false) end
        return
      end
      ...
      if entity_type == "Frame" or entity_type == "Thread" or entity_type == "Session" then
        ...
        E.report(E.warn("DapFocus: Cannot focus entity of type '" .. entity_type .. "'"))
        if callback then callback(false) end
      end
    end)
  end

  E.create_command("DapFocus", function(opts)
    api.focus(opts.args)
  end, {
    nargs = "+",
  ...
  function api.pick_config(callback)
    ...
    if #configs == 0 then
      error(E.warn("No active configs"), 0)
    end
  ...
  function api.pick_target(callback)
    ...
    if #items == 0 then
      error(E.warn("No active targets"), 0)
    end
  ...
  E.create_command("DapConfigs", function()
    api.pick_config()
  end, {
    desc = "Pick a Config to focus",
  })

  E.create_command("DapTargets", function()
    api.pick_target()
  end, {
