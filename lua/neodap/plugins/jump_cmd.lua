...
local function dap_jump(debugger, config)
  ...
  local function jump_to_frame(frame)
    ...
    if not loc then
      error(E.warn("DapJump: Frame has no source location"), 0)
    end
    ...
    elseif current_is_fixed then
      ...
      elseif strategy == "silent" then
        ...
        error(E.warn("DapJump: Current window is fixed (winfixbuf) — use a different window"), 0)
      end
    end
  ...
  function api.jump(target, callback)
    if not target or target == "" then
      local err = E.warn("DapJump: Missing target URL or URI")
    ...
    picker:resolve(target, function(frame)
      if not frame then
        E.report(E.warn("DapJump: Could not resolve target '" .. tostring(target) .. "'"))
        if callback then callback(false) end
        return
      end
      ...
      if frame:type() ~= "Frame" then
        E.report(E.warn("DapJump: Target is not a frame (got " .. frame:type() .. ")"))
        if callback then callback(false) end
        return
      end
  ...
  -- Create user command
  E.create_command("DapJump", function(opts)
