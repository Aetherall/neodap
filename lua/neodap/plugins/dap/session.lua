...
---Evaluate expression in global scope (no frameId)
---Used as fallback when no stopped frame is available.
---@param self neodap.entities.Session
---@param expression string
---@param opts? { silent?: boolean, context?: string }
---@return string result, number? variablesReference, string? type
function Session:evaluate(expression, opts)
  local dap_session = get_dap_session(self)
  if not dap_session then error("No DAP session", 0) end
  ...
  if not ok then
    if not opts.silent then
      Output.create(self, "❌ " .. tostring(body) .. "\n", "stderr")
    end
    error(body, 0)
  end

  if not opts.silent then
    Output.create(self, expression .. " → " .. tostring(body.result) .. "\n", "repl",
      { variablesReference = body.variablesReference })
  end

  return body.result, body.variablesReference, body.type
end
Session.evaluate = a.fn(Session.evaluate)

---Get completions in global scope (no frameId)
---Used as fallback when no stopped frame is available.
---@param self neodap.entities.Session
...
function Session:completions(text, column)
  local dap_session = get_dap_session(self)
  if not dap_session then error("No DAP session", 0) end
  ...
  end, "Session:completions")

  return body and body.targets or {}
end
Session.completions = a.fn(Session.completions)
