...
function SourceBinding:syncBreakpoints()
  ...
  -- Note: enabled=nil is treated as enabled (for backwards compatibility)
  local breakpoints = {}
  local bp_entities = {}
  for bp in source.breakpoints:iter() do
    -- Skip disabled breakpoints (enabled == false)
    if bp.enabled:get() == false then
      goto continue
    end
    ...
    if bp.condition:get() then
      bp_data.condition = bp.condition:get()
    end
    if bp.hitCondition:get() then
      bp_data.hitCondition = bp.hitCondition:get()
    end
    if bp.logMessage:get() then
      bp_data.logMessage = bp.logMessage:get()
    end
