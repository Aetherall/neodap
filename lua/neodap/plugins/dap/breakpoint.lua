...
-- Per-source sync mutexes (weak table keyed by source entity)
local source_mutexes = setmetatable({}, { __mode = "k" })
...
function SourceBinding:syncBreakpoints()
  ...
  -- Get or create mutex for this source to serialize concurrent syncs
  if not source_mutexes[source] then
    source_mutexes[source] = a.mutex()
  end
  local mtx = source_mutexes[source]
  a.wait(function(cb) mtx.lock(cb) end)

  -- Wrap all post-lock logic in pcall to guarantee mutex release on any error
  local ok, err = pcall(function()
    ...
    for bp in source.breakpoints:iter() do
      ...
      local bp_data = {
        ...
        condition = binding and binding:getEffectiveCondition() or bp.condition:get(),
        hitCondition = binding and binding:getEffectiveHitCondition() or bp.hitCondition:get(),
        logMessage = binding and binding:getEffectiveLogMessage() or bp.logMessage:get(),
      }
