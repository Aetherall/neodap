...
function Frame:fetchScopes()
  -- Label frames (await, Promise.then) are decorative async separators with no
  -- execution context — the adapter can't provide scopes for them.
  if self:isSkippable() then return end
...
function Frame:evaluate(expression, opts)
  ...
  if not ok then
    if not opts.silent then
      Output.create(session, "❌ " .. tostring(body) .. "\n", "stderr")
    end
    error(body, 0)
  end

  if not opts.silent then
    Output.create(session, expression .. " → " .. tostring(body.result) .. "\n", "repl",
