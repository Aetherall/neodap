...
return function(debugger, config)
  ...
  function api.step(method, granularity, url)
    ...
    if #entities == 0 then
      error(E.warn("DapStep: No thread found"), 0)
    end

    local step_fn = method_map[method]
    if not step_fn then
      error(E.warn("DapStep: Unknown method '" .. tostring(method) .. "'"), 0)
    end

    local count = apply_to_entities(entities, "Thread", step_fn, { granularity = granularity })

    if count == 0 then
      error(E.warn("DapStep: No threads to step"), 0)
    end
  end
  ...
  E.create_command("DapStep", function(opts)
