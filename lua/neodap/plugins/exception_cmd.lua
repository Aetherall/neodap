...
return function(debugger)
  ...
  E.create_command("DapException", function(opts)
    ...
    if not subcommand then
      ...
      local session = get_session()
      if not session then
        error(E.warn("No exception filters available"), 0)
      end
      ...
      vim.notify("[neodap] " .. table.concat(lines, "\n"), vim.log.levels.INFO)
      return
    end
    ...
    if subcommand == "toggle" then
      ...
      if not filter_id then
        error(E.warn("DapException: toggle requires filter ID"), 0)
      end
      local success
      if is_bang then
      ...
      if not success then
        error(E.warn("DapException: filter not found '" .. filter_id .. "'"), 0)
      end

    elseif subcommand == "enable" then
      local filter_id = args[2]
      if not filter_id then
        error(E.warn("DapException: enable requires filter ID"), 0)
      end
    ...
    elseif subcommand == "disable" then
      local filter_id = args[2]
      if not filter_id then
        error(E.warn("DapException: disable requires filter ID"), 0)
      end
    ...
    elseif subcommand == "clear" then
      local filter_id = args[2]
      if not filter_id then
        error(E.warn("DapException: clear requires filter ID"), 0)
      end
    ...
    elseif subcommand == "condition" then
      ...
      if not filter_id or not condition then
        error(E.warn("DapException: condition requires filter ID and expression"), 0)
      end
      ...
      error(E.warn("DapException: unknown subcommand '" .. tostring(subcommand) .. "'"), 0)
    end
  end, {
    nargs = "*",
    bang = true,
