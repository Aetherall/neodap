...
--   Lua: require("neodap.plugins.replline").open()
...
return function(debugger, config)
  ...
  local function open()
    ...
    -- Load dap-eval buffer via :edit (triggers BufReadCmd for proper setup)
    local uri = "dap-eval:@frame?closeonsubmit"
    vim.cmd.edit(uri)
  ...
  debugger:on_dispose(function()
    close()
    pcall(vim.api.nvim_del_user_command, "DapReplLine")
