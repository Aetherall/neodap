...
return {
  ...
  plugin = function(api)
    ...
    -- Helper function to get buffer number for breakpoint
    local function get_bufnr_for_breakpoint(breakpoint)
    ...
    breakpoint_manager.onBreakpoint(function(breakpoint)
      ...
      nio.run(function()
        local bufnr = get_bufnr_for_breakpoint(breakpoint).wait()
        ...
        breakpoint:onHit(function(_hit)
          nio.run(function()
            local hit_bufnr = bufnr
            if not hit_bufnr then
              hit_bufnr = get_bufnr_for_breakpoint(breakpoint).wait()
            end
            ...
            if hit_bufnr then
              when_buffer_loaded(hit_bufnr, function()
                ...
                local display = get_breakpoint_display(breakpoint, current_binding, true)
