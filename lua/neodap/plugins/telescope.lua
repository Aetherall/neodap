...
--   :DapPick @session/threads            - Pick from any URL-resolved collection
--   :DapPick /sessions(state=stopped)    - Pick from filtered entities
...
return function(debugger, config)
  ...
  function api.frames(opts)
    ...
    if #frames == 0 then
      error(E.warn("No frames in current stack"), 0)
    end
  ...
  ---Open a telescope picker for entities resolved from a URL path.
  ---Works with any URL that debugger:queryAll can resolve to a collection.
  ---@param url_str string URL path (e.g., "@session/threads", "/sessions(state=stopped)")
  ---@param opts? table Telescope picker options
  function api.pick_url(url_str, opts)
    opts = opts or {}

    local results = debugger:queryAll(url_str)
    if not results or #results == 0 then
      error(E.warn("DapPick: no results for '" .. url_str .. "'"), 0)
    end

    pickers.new(opts, {
      prompt_title = "Pick: " .. url_str,
      finder = finders.new_table({
        results = results,
        entry_maker = function(entity)
          return {
            value = entity,
            display = debugger:render_text(entity, { "title" }),
            ordinal = debugger:render_text(entity, { "title" }),
          }
        end,
      }),
      sorter = conf.generic_sorter(opts),
  ...
  E.create_command("DapPick", function(opts)
    local name = opts.args
    if api[name] then
      api[name]()
    else
      -- Treat as URL: resolve and present in generic picker
      api.pick_url(name)
    end
  end, {
    nargs = 1,
    ...
    complete = function(arglead)
      ...
      -- URL completions for arbitrary queries
