...
function M.goto_location(path, line, column, options)
  ...
  -- For virtual DAP buffers, content loads asynchronously (initially just "-- Loading...").
  -- Set dap_pending_cursor so source_buffer's apply_pending_cursor() repositions
  -- the cursor after the async content load completes.
  local is_virtual = path:match("^dap://")
  if is_virtual and line > buf_line_count then
    vim.b[bufnr].dap_pending_cursor = { line = line, col = math.max(0, column) }
  end
