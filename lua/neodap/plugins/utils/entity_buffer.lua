...
function M.submit(bufnr)
  ...
  if not reg or not reg.opts.submit then
    error(E.warn("This buffer does not support editing"), 0)
  end
  ...
  if not entity then
    error(E.warn("No entity bound to this buffer"), 0)
  end
  ...
  if ok then
    ...
    E.report(E.user("Submit failed: " .. tostring(err)))
  end
end
...
--- List all registered URI schemes (for completion)
---@return string[] schemes e.g., {"dap://tree", "dap://console", "dap://input", ...}
function M.registered_schemes()
  local schemes = {}
  for scheme in pairs(registry) do
