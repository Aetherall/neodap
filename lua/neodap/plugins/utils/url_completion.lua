...
local function ensure_schema_info()
  ...
  -- Flat schema format: { TypeName = { field = "type" | { type = "edge", ... } | ... } }
  for type_name, type_def in pairs(schema.schema) do
    local info = { edges = {}, rollups = {}, properties = {} }

    for field_name, field_def in pairs(type_def) do
      if type(field_def) == "string" then
        -- Property: name = "string", state = "string", etc.
        info.properties[field_name] = field_def
      elseif type(field_def) == "table" then
        local ft = field_def.type
        if ft == "edge" then
          info.edges[field_name] = { target = field_def.target, indexes = field_def.__indexes }
          edge_to_type[field_name] = field_def.target
        elseif ft == "reference" then
          -- Reference rollups resolve through their source edge
          info.rollups[field_name] = field_def
          if field_def.edge then
            local source_edge = type_def[field_def.edge]
            if source_edge and type(source_edge) == "table" and source_edge.type == "edge" then
              edge_to_type[field_name] = source_edge.target
            end
          end
        elseif ft == "collection" then
          -- Collection rollups resolve through their source edge
          info.rollups[field_name] = field_def
          if field_def.edge then
            local source_edge = type_def[field_def.edge]
            if source_edge and type(source_edge) == "table" and source_edge.type == "edge" then
        ...
        elseif ft == "count" then
          -- Count rollups are property-like (return numbers)
          info.rollups[field_name] = field_def
        end
      end
    end
