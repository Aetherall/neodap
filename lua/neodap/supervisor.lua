...
local function create_rundir()
  ...
  -- Use os.execute instead of vim.fn.mkdir to avoid E739 when parent dirs exist
  -- (vim.fn.mkdir with "p" flag can error on existing directories in some Neovim versions)
...
---@field stop fun() Kill entire process tree (adapter + debuggees). Use for terminate.
---@field disconnect fun() SIGTERM shim only — adapter dies, debuggees in own sessions survive. Use for disconnect.
...
---@field stop fun() Kill entire compound process tree (all adapters + debuggees). Use for terminate.
---@field disconnect fun() SIGTERM compound shim — cascades to config shims and adapters, debuggees survive. Use for disconnect.
...
local function spawn_shim(script, args, env, cwd)
  ...
  local handle, pid = uv.spawn("setpriv", {
    ...
    -- Close the handle to reap the zombie process
...
function M.launch_config(opts, callback)
  ...
  local handle = {
    ...
    disconnect = function()
      -- Graceful: SIGTERM the shim, which forwards to adapter via trap.
      -- Debuggees that called setsid() (like node) survive.
      pcall(uv.kill, pid, "sigterm")
    end,
    kill = function()
...
function M.launch_compound(opts, on_config_ready, on_config_error)
  ...
  for _, cfg in ipairs(opts.configs) do
    ...
    local cfg_handle = {
      ...
      disconnect = function()
        -- Graceful: SIGTERM the config shim only. The trap handler kills the
        -- adapter child, but debuggees in their own sessions survive.
        local pid_file = config_dir .. "/pid"
        local f_pid = io.open(pid_file, "r")
        if f_pid then
          local cfg_pid = tonumber(f_pid:read("*a"))
          f_pid:close()
          if cfg_pid then pcall(uv.kill, cfg_pid, "sigterm") end
        end
      end,
      kill = function()
        local pid_file = config_dir .. "/pid"
  ...
  local compound_handle = {
    ...
    disconnect = function()
      -- Graceful: SIGTERM compound shim, which traps and forwards to all config shims.
      -- Config shims forward to adapters. Debuggees in own sessions survive.
      pcall(uv.kill, pid, "sigterm")
    end,
    kill = function()
      kill_tree(pid, "sigkill")
