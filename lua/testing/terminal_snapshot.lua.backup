...
local function format_screen_for_embedding(screen, name)
  ...
  table.insert(lines,
    "Cursor: [" ..
    screen.cursor[1] .. ", " .. screen.cursor[2] .. "] (line " .. screen.cursor[1] .. ", col " .. screen.cursor[2] .. ")")
  ...
  --     if string.starts(hl.name, "Neodap") then
  --       local range_str
  ...
  for i, line in ipairs(screen.lines) do
    ...
    -- Replace [[ with [{ and ]] with }] to avoid breaking Lua comments
    visible_line = visible_line:gsub("%[%[", "[{")
    visible_line = visible_line:gsub("%]%]", "}]")
