...
--- Check if item has any expanded edges that actually contain children.
--- Unlike any_expanded(), returns false for empty expansions (e.g., leaf Variables
--- whose children edge is eagerly expanded but has count=0).
function neo.Item:any_expanded_with_children()
  local pk = path_to_key(self._path)
  local exp = self._view.expansions[pk]
  if not exp then return false end
  for edge_name, edge_data in pairs(exp) do
    if edge_data.count and edge_data.count > 0 then return true end
  end
  return false
end
