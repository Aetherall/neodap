...
4. [Features](#features)
5. [The Primitives](#the-primitives)
6. [How They Compose](#how-they-compose)
7. [The Built-in Plugins](#the-built-in-plugins)
...
Set breakpoints, step through code, and inspect state. Frame highlights, inline values, and breakpoint signs update reactively as you navigate.

![Debugging](docs/recordings/core-debug-ux.gif)

### Variable Inspection

Variables and Call Stack panels appear automatically when stopped. Expand scopes, drill into objects and arrays — the tree updates reactively as the debug context changes.

![Variable Inspection](docs/recordings/tree_buffer.gif)

### Tree Navigation

Navigate the tree with `l` to expand, `h` to go to parent, and `<CR>` to toggle. Expand variables to drill into objects, or expand Call Stack frames to inspect their scopes inline.

![Tree Navigation](docs/recordings/tree_navigation.gif)

### Hover

Hover over any expression to see its runtime value. neodap registers as an in-process LSP hover provider, so `K` shows both debug values and language server info in a single popup.

![Hover](docs/recordings/hover.gif)

### Multi-Session

Run multiple debug sessions simultaneously. The session picker shows all active sessions — select one to switch focus. Variables, Call Stack, and source view update instantly.

![Multi-Session](docs/recordings/multi_session.gif)

### Console

View program output, evaluation results, and debug messages. The console buffer tracks the focused session and its ancestors.

![Console](docs/recordings/console_buffer.gif)
...
| `rooter` | VS Code launch.json (via [rooter.nvim](https://github.com/Aetherall/rooter.nvim)) |
